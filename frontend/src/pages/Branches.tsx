...
export default function Branches() {
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await apiClient.exportBranches();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `branches_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to export branches',
        variant: 'destructive',
      });
    }
  };
